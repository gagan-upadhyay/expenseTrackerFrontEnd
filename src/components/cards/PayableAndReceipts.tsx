...
export default function PayablesReceiptsCard() {
  ...
  <div className="flex flex-col h-full relative z-10 overflow-hidden">
          ...
          {payables.length > 0 ? (
            payables.map((item) => (
              <SwipeableTransaction 
                swipeFor='payables'
                  key={item.id}
                  onDone={() => updateTransaction(item.id, item.account_id, {is_payable:false, occurred_at: new Date().toISOString() })}
    ...
    <ImagePreviewModal
      ...
      // ✅ Pass the ID and Account ID to your existing delete handler
