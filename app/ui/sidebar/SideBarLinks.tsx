...
export default function SideBarLinks({ className, theme }: UserProfileProps){
    ...
    return(
        <div className="flex flex-row md:flex-col md:items-center md:pb-11 md:mt-10 ml-3 ">
        {links.map((link)=>{
            ...
            return(
                ...
                key={link.name}
                href={link.href}
                className={clsx('flex px-4 py-2 justify-center md:justify-start relative rounded-xl w-[90%]',
                pathname===link.href?
                (theme==='dark'?
        ...
        <div className="hidden md:block md:w-[80%] border-b-1 mt-5 ml-[-20]">
        </div>
        </div>
